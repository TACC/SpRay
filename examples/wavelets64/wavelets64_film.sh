...
then
  ...
  then
    NUM_MPI_TASKS=2
    NUM_THREADS=1
    COMMAND="mpirun -n $NUM_MPI_TASKS $SPRAY_BIN/spray_insitu --ply-path $PLY_PATH --nthreads $NUM_THREADS -w 512 -h 512 --frames 1 --mode film --cache-size -1 --partition insitu --camera 90.172180 84.141418 82.480225 30.000000 28.649426 30.000000 --pixel-samples 1 --ao-samples 1 --bounces 1 --num-partitions 8 --blinn 0.4 0.4 0.4 10 $MODEL"
  elif [ "$app" == "4" ]
  then
    NUM_MPI_TASKS=1
    NUM_THREADS=4
    COMMAND="mpirun -n $NUM_MPI_TASKS $SPRAY_BIN/baseline_ooc --ply-path $PLY_PATH --nthreads $NUM_THREADS -w 512 -h 512 --frames 1 --mode film --cache-size -1 --partition image --camera 90.172180 84.141418 82.480225 30.000000 28.649426 30.000000 --pixel-samples 1 --ao-samples 1 --bounces 1 --num-partitions 8 --blinn 0.4 0.4 0.4 10 $MODEL"
  else
    NUM_MPI_TASKS=1
    NUM_THREADS=1
    COMMAND=""
