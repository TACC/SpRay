...
inline void genMultiSampleEyeRays2(const Camera& camera, int image_w,
                                   int image_h, float orgx, float orgy,
                                   float orgz, int num_pixel_samples,
                                   spray::Tile blocking_tile, spray::Tile tile,
                                   RayBuf<Ray>* ray_buf) {
  Ray* rays = ray_buf->rays;
  ...
  for (int y = tile.y; y < tile.y + tile.h; ++y) {
    for (int x = tile.x; x < tile.x + tile.w; ++x) {
      for (int s = 0; s < num_pixel_samples; ++s) {
        ...
        ray->pixid = image_w * y + x;
#ifdef SPRAY_GLOG_CHECK
        CHECK_LT(ray->pixid, image_w * image_h);
#endif
